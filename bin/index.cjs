...
function read(file) {
  return (0, import_node_fs.readFileSync)((0, import_node_path.resolve)(__dirname, file), "utf8");
}
function write(dir, content) {
  (0, import_node_fs.writeFileSync)(dir, content, "utf8");
  return true;
}
function indexInsert(pattern, replacement) {
...
program2.name("get-docker-compose").description("Generate a docker-compose file").version("1.0.0").action(async () => {
  const { dir, services } = await input_default();
